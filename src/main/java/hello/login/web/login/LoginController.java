...
public class LoginController {
    ...
    public String login(@Valid @ModelAttribute LoginForm form, BindingResult bindingResult,
        ...
        if (loginMember == null) {
            bindingResult.reject("loginFail", "아이디 또는 비밀번호가 맞지 않습니다."); //global오류
            return "login/loginForm";
        }
   ...
   // @PostMapping("/login")
     ...
     *   로그인하면 나중에 기존에 요청했던 url 로 redirect 해주는 기능 추가
     */
    @PostMapping("/login")
    public String loginV4(@Valid @ModelAttribute LoginForm form, BindingResult bindingResult,
        ...
        return "redirect:" + redirctURL; // 없으면 default "/"로 이동
    ...
    public String logoutV3(HttpServletRequest request) {
        HttpSession session = request.getSession(false); // false 일때 세션이 없으면 null 반환
        if (session != null) {
            session.invalidate();
        }
        return "redirect:/";
